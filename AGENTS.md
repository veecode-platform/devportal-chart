...
| charts/backstage/README.md.gotmpl | The title, TL;DR, prerequisites, usage, and Orchestrator sections describe VeeCode DevPortal from next-charts; Orchestrator uses external PostgreSQL, and the cluster-wide orchestrator-infra prerequisite comes from the Red Hat Developer Hub chart repository. | Keep the VeeCode install commands, external database contract, and external prerequisite source when refreshing the template. |
